...
<template>
  ...
  >
    <MySpin
      class="m-img"
      :style="`width: ${property.width}px; height: ${property.height}px; top: ${property && property.top}px; left: ${
      ...
      :key="index"
    >
      <img class="u-img" :src="images[index].src" :alt="images[index].title" @load="onLoaded(index)" />
    </MySpin>
  </div>
  <div
    v-if="mode === 'CSS'"
  ...
  >
    ...
    >
      <div class="item-box">
        ...
        <p class="p-price">￥13900.00</p>
        <p> 现代简约牛皮沙发三人位 Land意大利皮革皮艺沙发 【布料】颜色可选 双人位M【180cm】</p>
        <p>5条评价</p> -->
        <img class="u-img" :src="item.src" :alt="item.title" @load="onLoaded(index)" />
